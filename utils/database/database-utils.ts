...
export async function findUser(email:string, googleID: string) {
  try {
    ...
    const usersCollection = client.db(process.env.DATABASE_NAME).collection("users")
    const selectedUser = await usersCollection.findOne({email: email, googleID: googleID })
    return selectedUser;
  } 
  catch (error) {
      console.error("something went wrong when searching for a user in the database", error)
  }
  finally {
    await client.close();
  }
...
export async function createUserIfNotExists(email:string, googleID: string) {
  ...
  try {
    ...
    const usersCollection = client.db(process.env.DATABASE_NAME).collection("users")
    const userExists = await usersCollection.findOne({email: email, googleID: googleID })
    let selectedUser = null;
    if (userExists !== null) {
        console.log("Attempted to create user, but user already exists. returning user...");
      ...
      console.error("something went wrong when creating a user in the database", error)
  }
  finally {
    await client.close();
  return selectedUser
